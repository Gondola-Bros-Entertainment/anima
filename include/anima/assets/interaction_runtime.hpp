...
/// `std::invalid_argument` unless stated. JSON syntax errors, missing role or phase entries and
/// values of the wrong JSON type throw the private JSON parser's exceptions, which derive only
/// from `std::exception`.
